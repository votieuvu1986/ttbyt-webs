...
observeEvent(input$help_summary, {
  ...
  if (input$chart_summary == "Spine") {
    ...
    showModal(modalDialog(
      title = "Sử dụng biểu đồ như thế",
      p(img(src="help_barcode2.png",
            height=600)),
      size = "l",
      easyClose = TRUE, 
      fade=FALSE,
      footer = modalButton("Đóng (Esc)")
    ))
    ...
    showModal(modalDialog(
      ...
      h5("Area comparison example:  e.g. comparing 'Area X' against Scotland", 
         style = "font-weight: bold; color: black; margin-bottom: 0px;"),
      ...
      h5("Time comparison example: e.g. yearly comparisons of 'Area X' against the value for year 2003", 
         style = "font-weight: bold; color: black; margin-bottom: 0px;"),
...
output$defs_text_summary <- renderUI({
  ...
  HTML(paste(sprintf("<b><u>%s</b></u> <br> %s ", 
                     defs_data_summary()$indicator_name, 
                     defs_data_summary()$indicator_definition), 
             collapse = "<br><br>"))
})
...
output$save_chart_ui <- renderUI({
  if (input$chart_summary %in% c("Trend", "Snapshot") ){
    savechart_button('download_summaryplot_no', 
                     'Lưu biểu đồ',  
                     class = "down", 
                     disabled=TRUE)
  } else if (input$chart_summary == "Spine") {
    savechart_button('download_summaryplot', 
                     'Lưu biểu đồ',  
                     class = "down")
  }
...
output$geoname_ui_summary <- renderUI({
  ...
  areas_summary <- if (input$geotype_summary %in% c("Health board", 
                                                    "Council area", 
                                                    "HSC partnership", 
                                                    "Scotland", 
  ...
  selectizeInput("geoname_summary", 
                 label = NULL,  
...
output$comp_ui_summary <- renderUI({
  ...
  if (input$chart_summary %in% c("Snapshot", "Trend")) {
    if (input$comp_summary == 1) {
      selectInput("geocomp_summary", 
                  "Select a comparison area", 
                  choices = comparator_list,
                  selectize=TRUE, 
                  selected = "Scotland")
      ...
      selectizeInput("yearcomp_summary", 
                     "Baseline year", 
                     choices = years)
    }
  } else if (input$chart_summary == "Spine") {
    selectizeInput("geocomp_spine", 
                   "Select a comparison area", 
                   choices = comparator_list, 
                   selected = "Scotland")
  }
...
summary_data <- reactive({
  ...
  summary_chosen_area <- optdata %>% 
    subset(areaname == input$geoname_summary &
             ...
             !(indicator %in% c("Mid-year population estimate - all ages", 
                                "Quit attempts")) &
...
output$summary_ui_plots <- renderUI({
  ...
  if (input$chart_summary == "Snapshot") {
    ...
    tagList(
      withSpinner( plotOutput("spine_plot", 
                              height=spine_plot_height(), 
                              width="90%")),
      br(), br()
    )
...
summary_csv <- reactive({ 
  if (input$chart_summary == "Snapshot") {
    format_csv(snapshot_data(), extra_vars = "comp_m") %>%
      mutate(comparator_name = ifelse(input$comp_summary == 1, 
                                      input$geocomp_summary,
...
output$download_summaryplot <- downloadHandler(
  ...
  content = function(file){
    if (input$chart_summary == "Snapshot") {
      ...
      ggsave(file, plot = plot_heat()+
               ggtitle(paste0(names(profile_list[unname(profile_list) == input$profile_summary]),
                       ...
                       subtitle =       if(input$comp_heat == 1){
                         paste0(input$geoname_heat," (", 
                                input$geotype_heat, 
                                ") compared against ",
                                input$geocomp_heat)
                       } else if(input$comp_heat==2){
                         paste0("Changes within ",
                                input$geoname_heat,
      ...
      ggsave(file, 
             plot = plot_spine()+
             ggtitle(label=paste(names(profile_list[unname(profile_list) == input$profile_summary]),
                                 " profile: ", 
                                 input$topic_spine,
                                 sep=""),
                       subtitle =paste(input$geoname_summary," (",
                                       input$geotype_summary,
                                       ") compared against ",
                                       input$geocomp_spine,
                                       sep="")),
             device = "png", 
...
plot_profile_snapshot <- function(domainchosen) {
  ...
  if (is.data.frame(prof_snap_data) && nrow(prof_snap_data) == 0)
  {
    ...
    if (input$comp_summary == 1) {#depending if time or area comparison
      tooltip_summary <-  c(paste0("Area: ", 
                                   round(prof_snap_data$measure, 1), " || ",
                                   "Comparator: ", 
                                   round(prof_snap_data$comp_m, 1), "<br>",
                                   prof_snap_data$trend_axis, "<br>", 
    ...
    plot_ly(prof_snap_data, 
            y = ~indicator,   
            color = ~color, 
            height = height_plot,
            colors=  c(blue = "#4da6ff", 
                       gray = "gray88", 
                       red = "#ffa64d", 
                       white = "#ffffff")
    ) %>%
      add_bars(x =1, 
               ...
               width=1,
               hoverinfo="text", 
               hovertext = tooltip_summary,
               marker = list(line= list(color="black", width = 0.5))) %>%
      # adding indicator name at center of each bar
      add_text(text = ~indic_multiline, 
               x =0.5,  
               showlegend= FALSE,
               textfont = list(color='black'), 
               hoverinfo="skip" ) %>%
      layout(yaxis = axis_layout, 
             xaxis = axis_layout,
...
plot_heat <- function(domain_plot){
  ...
  if (is.data.frame(heat) && nrow(heat) == 0)
  {
    ...
    #Tooltip
    heat_tooltip <- paste0(heat$indicator, "<br>", 
                           heat$def_period, "<br>",
                           heat$type_definition, "<br>", 
                           round(heat$measure, 1))
    
    # Plotting data
    heat_p <- ggplot(heat, aes(x = year, 
                               y = indicator, 
                               fill = color, 
                               text = heat_tooltip)) +
      geom_tile(color = "black") +
      geom_text(aes(label = round(measure, 0)), 
                size =3) +
      ...
      scale_fill_manual(name = "Legend", 
                        labels = c("Significantly better", 
                                   "Not significantly different", 
                                   "Significantly worse", 
                                   "Significance is not calculated"),
                        values = c(blue = "#4da6ff", 
                                   gray = "gray88", 
                                   red = "#ffa64d", 
                                   white = "#ffffff")) +
      #Giving the right dimensions to the plot
      scale_x_continuous(position = "top", 
                         breaks=seq(from = min(summary_data()$year), 
                         to = 2018, 
                         by =1)) + #max(summary_data()$year)
      #Layout
    ...
    ggplotly(heat_p, 
      ...
      layout(margin = list(l = 400, 
             ...
             font = font_plots) %>%
      config(displayModeBar = FALSE, 
             displaylogo = F) # taking out plotly logo 
  }
}
...
spine_allareas <- reactive({
  ...
  optdata %>%
    ...
    subset (year == max_year &
              (substr(profile_domain1, 1, 3) == input$profile_summary |
                 substr(profile_domain2, 1, 3) == input$profile_summary) &
              (substr(profile_domain1, 5, 
                      nchar(as.vector(profile_domain1))) == input$topic_spine |
                 substr(profile_domain2, 5, 
                        nchar(as.vector(profile_domain2))) == input$topic_spine) &
              areatype  == input$geotype_summary &
              !(indicator %in% c("Mid-year population estimate - all ages", 
                                 "Quit attempts")))
})
...
plot_spine <- function(){
  ...
  #Merging comparator and chosen area
  spine <- merge(spine_allareas(), 
                 spine_chosencomp(), 
                 by=c("indicator"))
  spine <- left_join(spine, 
                     spine_chosenarea(), 
                     by=c("indicator"))
  ...
  data_labels <- spine %>%
    ...
    group_by(indicator, 
             ...
             trend_axis, 
             code) %>%
    summarise(comp_lab=measure_comp[1], 
              chosen_lab=measure_chosen[1],
              x_chosen=chosen2[1], 
              x_comp = comp[1])%>%
    droplevels()
  ...
  colour_lines <-  scale_colour_manual(" ",
                                       values= setNames(c("black","#009999","#990000"), 
                                                        c(areatype_name, 
                                                          chosenarea_name, 
                                                          comparea_name)))
  ...
  colour_points <- scale_fill_manual(name = "flag",
                                     values = fillcolours)
  ...
  ggplot(spine_data, 
         aes(x = all2, 
             ...
             colour=areatype))+
    geom_line(alpha=0.6)+
    geom_line(aes(x = chosen2, 
                  colour=chosenarea_name), 
              size=1) + #line for picked area
    geom_line(aes(x = comp, 
                  colour=comparea_name), 
              size=1) + #line for comparator
    ...
    geom_text(data=data_labels, 
              aes(x=x_chosen,
                  y=1.5,
                  label=round(chosen_lab,digits=0)),
              check_overlap = TRUE,
              size=5,
              colour = "#009999",
              vjust=1, 
              hjust=0) + #label for chosenarea - teal
    geom_text(data=data_labels, 
              aes(x=x_comp,
                  y=-0.6,
                  label=round(comp_lab,digits=0)),
              check_overlap = TRUE,
              size=5,
              colour = "#990000",
              vjust=0, 
              hjust=1) + #label for comparator - red
    xlab("Worse  <-------------------->  Better")+
    ...
    theme(
      ...
      axis.title.x=element_text(size=12, 
                                colour ='#555555',
                                hjust=0.6), #x axis title contains better/worse
    ...
    facet_wrap(~indicator + type_definition + 
                 trend_axis, # fields to facet on
...
output$ui_spine_legend_areatype <- renderUI({
  ...
  text_legend <- case_when(input$geotype_summary == "Scotland" ~ "Scotland",
                           TRUE ~ paste0("Other ", 
                                         tolower(input$geotype_summary), "s"))
  
  img(src='bar_legend_areatype.jpg', 
      height=18, 
      style="padding-right: 2px; vertical-align:middle", 
      text_legend)
}) 
...
spine_csv <- reactive({
  ...
  spine %>%
    select(c(indicator, areaname, areatype, 
             def_period, numerator, measure,
             lowci, upci, type_definition, 
             topic, measure_comp, comparator, flag)) %>%
    rename(lower_confidence_interval=lowci, 
           upper_confidence_interval=upci,
           latest_period = def_period, 
           definition = type_definition, 
           comparator_measure = measure_comp, 
           difference=flag)
...
output$download_spineplot <- downloadHandler(
  ...
  content = function(file){
    ggsave(file, plot = plot_spine()
           +ggtitle(label=paste(names(profile_list[unname(profile_list) == input$profile_summary]),
                                " profile: ", 
                                input$topic_spine,sep=""),
                    subtitle =paste(input$geoname_summary," (",
                                    input$geotype_summary,") compared against ",
                                    input$geocomp_spine,sep="")),
           device = "png",width=15, 
           limitsize=FALSE)
  })

################################################################################
##### Các biểu đồ tổng quan của Trang bị Y tế #####


### Vẽ biểu đồ số lượng model theo các phân nhóm ###
tbyt_moldel <- tbyt %>% 
  mutate(dup1=duplicated(tbyt$tbyt_model)) %>%
  filter(dup1 %in% c("FALSE"))

output$summary_modelplot1 <- renderPlotly({
  # Số lượng model theo phân nhóm họ thiết bị y tế "tbyt_category" #
  gg1 <- ggplot(tbyt_moldel, aes(x=tbyt_category)) + 
    geom_bar(width=.6, fill=rainbow(length(table(tbyt_moldel$tbyt_category)))) + 
    #labs(title="Biểu đồ số lượng model của nhóm các thiết bị y tế", 
         #subtitle="Các nhóm thiết bị y tế phân theo họ thiết bị") +
    xlab("Các nhóm thiết bị y tế") +
    ylab("Số lượng model") + 
    theme(axis.text.x = element_text(angle=55, vjust=0.5))
  ggplotly(gg1, height=600)
})

output$summary_modelplot2 <- renderPlotly({
  #par(mfrow=c(2,1))
  # Số lượng model theo phân nhóm theo TT14 "tbyt_tt14_tbyt_group" #
  gg6 <- ggplot(tbyt_moldel, aes(x=tbyt_tt14_tbyt_group)) + 
    geom_bar(width=.6, fill=c("#4292c6", "#6baed6", "#084594", "#c6dbef", "#9ecae1", "#2171b5","#eff3ff")) + 
    #labs(title="Biểu đồ số lượng model theo phân loại Thông tư 14") +
    xlab("Nhóm phân loại") +
    ylab("Số lượng model") + 
    theme(axis.text.x = element_text(angle=55, vjust=0.5))
  ggplotly(gg6, height = 280)
})
  
output$summary_modelplot3 <- renderPlotly({  
  # Số lượng model theo phân nhóm rủi ro "tbyt_risk_level" #
  gg7 <- ggplot(tbyt_moldel, aes(x=tbyt_risk_level)) + 
    geom_bar(width=.6, fill=c("#ffeda0", "#feb24c", "#fc4e2a", "#bd0026", "#fcfbfd")) + 
    #labs(title="Biểu đồ số lượng model theo phân loại rủi ro") +
    xlab("Nhóm phân loại") +
    ylab("Số lượng model") + 
    theme(axis.text.x = element_text(angle=0, vjust=0.5))
  ggplotly(gg7, height = 280)
})


### Phân tích dữ liệu trang bị y tế theo thời gian ###
tbyt_moldel$date <- as.Date(tbyt_moldel$tbyt_date_create)
tbyt_moldel$month <- format.Date(tbyt_moldel$date, "%Y-%m")
tbyt_moldel$year <- format.Date(tbyt_moldel$date, "%Y")

# Khai báo thư viện bổ sung #
library(dygraphs)
library(xts)          
library(tidyverse)
library(lubridate)

# Biểu đồ số lượng model theo thời gian "date" #
data <- data.frame(table(tbyt_moldel$date))
data$date <- as.Date(data$Var1)
don <- xts(x = data$Freq, order.by = data$date)

output$summary_modelplot4 <- renderDygraph({  
p1 <- dygraph(don) %>%
...
output$summary_modelplot5 <- renderPlotly({
  # Biểu đồ số lượng model theo tháng "month" #
  p2 <- ggplot(tbyt_moldel, aes(month)) +
    geom_bar(fill=rainbow(length(table(tbyt_moldel$month))), alpha=0.85) +
    xlab("Thời gian") +
    ylab("Số lượng model")
  ggplotly(p2)
})

output$summary_modelplot6 <- renderPlotly({
  # Số lượng model theo phân nước sở hữu "tbyt_country_owner" #
  gg4 <- ggplot(tbyt_moldel, aes(x=tbyt_country_owner)) + 
    geom_bar(width=.6, fill = "#1E90FF") + 
    #scale_fill_manual(values = alpha(c("blue", "red"), .3)) +
    #labs(title="Biểu đồ số lượng model của nước sở hữu") +
    xlab("Các nước sở hữu") +
    ylab("Số lượng model") + 
    theme(axis.text.x = element_text(angle=60, vjust=0.5))
  ggplotly(gg4)
})

output$summary_modelplot7 <- renderPlotly({
  # Số lượng model theo phân nước xuất xứ "tbyt_country_origin" #
  gg5 <- ggplot(tbyt_moldel, aes(x=tbyt_country_origin)) + 
    geom_bar(width=.6, fill="#1E90FF") + 
    #labs(title="Biểu đồ số lượng model của nước xuất xứ") +
    xlab("Các nước xuất xứ") +
    ylab("Số lượng model") + 
    theme(axis.text.x = element_text(angle=60, vjust=0.5))
  ggplotly(gg5)
})

output$summary_modelplot8 <- renderPlot({
  
  # Số lượng model theo phân tình trạng khai báo VAT "tbyt_vat" #
  dta.status <- data.frame(table(tbyt_moldel$tbyt_vat))
  dta.status$fraction <- dta.status$Freq / sum(dta.status$Freq)
  dta.status$ymax <- cumsum(dta.status$fraction)
  dta.status$ymin <- c(0, head(dta.status$ymax, n=-1))
  dta.status$labelPosition <- (dta.status$ymax + dta.status$ymin) / 2
  dta.status$label <- paste0(c("Chưa có VAT", "Đã có VAT"), "\n Số lượng: ", dta.status$Freq)
  
  # Số lượng model theo phân nước xuất xứ "tbyt_country_origin" #
  gg8 <- ggplot(dta.status, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Var1)) +
    geom_rect() +
...
output$summary_modelplot9 <- renderPlot({
  
  # Số lượng model theo nhóm thiết bị y tế phục vụ chống dịch COVID19 "tbyt_group_covid19" #
  dta.status <- data.frame(table(tbyt_moldel$tbyt_group_covid19))
  dta.status$fraction <- dta.status$Freq / sum(dta.status$Freq)
  dta.status$ymax <- cumsum(dta.status$fraction)
  dta.status$ymin <- c(0, head(dta.status$ymax, n=-1))
  dta.status$labelPosition <- (dta.status$ymax + dta.status$ymin) / 2
  dta.status$label <- paste0(c("Trang bị thường", "Chống dịch COVID19"), "\n Số lượng: ", dta.status$Freq)
  
  # Số lượng model theo phân nước xuất xứ "tbyt_country_origin" #
  gg9 <- ggplot(dta.status, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Var1)) +
    geom_rect() +
    geom_text( x=2, aes(y=labelPosition, label=label, color=Var1), size=6) + 
    coord_polar(theta="y") +
    xlim(c(1, 4)) +
...
########################################################
##### Các biểu đồ tổng quan của Vật tư y tế #####

### Phân tích dữ liệu trang bị y tế theo thời gian ###
tbyt$date <- as.Date(tbyt$tbyt_date_create)
tbyt$month <- format.Date(tbyt$date, "%Y-%m")
tbyt$year <- format.Date(tbyt$date, "%Y")

output$summary_countplot1 <- renderDygraph({
  # Biểu đồ số lượng model theo thời gian "date" #
  data <- data.frame(table(tbyt$date))
  data$date <- as.Date(data$Var1)
  don <- xts(x = data$Freq, order.by = data$date)
...
output$summary_countplot2 <- renderPlotly({
  # Biểu đồ số lượng model theo tháng "month" #
  p2 <- ggplot(tbyt, aes(month)) +
    geom_bar(fill=rainbow(length(table(tbyt$month))), alpha=0.85) +
    xlab("Thời gian") +
    ylab("Số lượt khai báo")
  ggplotly(p2)
})

output$summary_countplot3 <- renderPlotly({
  # Số lượng model theo phân nhóm họ thiết bị y tế "tbyt_category" #
  gg1 <- ggplot(tbyt, aes(x=tbyt_category)) + 
    geom_bar(width=.6, fill=rainbow(length(table(tbyt$tbyt_category)))) + 
    #labs(title="Biểu đồ số lượng model của nhóm các thiết bị y tế", 
    #subtitle="Các nhóm thiết bị y tế phân theo họ thiết bị") +
    xlab("Các nhóm thiết bị y tế") +
    ylab("Số lượt khai báo") + 
...
output$summary_countplot4 <- renderPlotly({
  #par(mfrow=c(2,1))
  # Số lượng model theo phân nhóm theo TT14 "tbyt_tt14_tbyt_group" #
  gg6 <- ggplot(tbyt, aes(x=tbyt_tt14_tbyt_group)) + 
    geom_bar(width=.6, fill=c("#4292c6", "#6baed6", "#084594", "#c6dbef", "#9ecae1", "#2171b5","#eff3ff")) + 
    #labs(title="Biểu đồ số lượng model theo phân loại Thông tư 14") +
    xlab("Nhóm phân loại") +
    ylab("Số lượt khai báo") + 
...
output$summary_countplot5 <- renderPlotly({  
  # Số lượng model theo phân nhóm rủi ro "tbyt_risk_level" #
  gg7 <- ggplot(tbyt, aes(x=tbyt_risk_level)) + 
    geom_bar(width=.6, fill=c("#ffeda0", "#feb24c", "#fc4e2a", "#bd0026", "#fcfbfd")) + 
    #labs(title="Biểu đồ số lượng model theo phân loại rủi ro") +
    xlab("Nhóm phân loại") +
    ylab("Số lượt khai báo") + 
...
output$summary_countplot6 <- renderPlotly({
  # Số lượng model theo phân nước sở hữu "tbyt_country_owner" #
  gg4 <- ggplot(tbyt, aes(x=tbyt_country_owner)) + 
    geom_bar(width=.6, fill = "#1E90FF") + 
    #scale_fill_manual(values = alpha(c("blue", "red"), .3)) +
    #labs(title="Biểu đồ số lượng model của nước sở hữu") +
    xlab("Các nước sở hữu") +
...
output$summary_countplot7 <- renderPlotly({
  # Số lượng model theo phân nước xuất xứ "tbyt_country_origin" #
  gg5 <- ggplot(tbyt, aes(x=tbyt_country_origin)) + 
    geom_bar(width=.6, fill="#1E90FF") + 
    #labs(title="Biểu đồ số lượng model của nước xuất xứ") +
    xlab("Các nước xuất xứ") +
    ylab("Số lượng model") + 
