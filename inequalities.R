#Code to inequalities tab
    ...
    showModal(modalDialog(
      ...
      p("You can read more about the measures used and presented in the",
        tags$a(href="https://www.scotpho.org.uk/comparative-health/measuring-inequalities/", #to change
               "Measuring inequalities section",  
               class="externallink"), 
  ...
  output$defs_text_simd <- renderUI({
    ...
    HTML(paste(sprintf("<b><u>%s</b></u> <br> %s ", 
                       defs_data$indicator_name, 
                       defs_data$indicator_definition), 
               collapse = "<br><br>"))
  })
  ...
  output$geoname_ui_simd <- renderUI({
    ...
    list_areas <- sort(as.vector(subset(geo_lookup$areaname, 
                                        geo_lookup$areatype == input$geotype_simd)))
    
    selectInput("geoname_simd", 
                "Select the area", 
                choices = list_areas, 
                selected = "Scotland")
  })
  ...
  output$year_ui_simd <- renderUI({
    ...
    selectInput("year_simd", "Bước 3 - Lựa chọn khoảng thời gian",
                choices = time_period, 
                selected = last(time_period))
  })
  ...
  simd_bar_data <- reactive({
    simd_quint_data() %>%
      subset(code %in% 
               as.character(geo_lookup$code[geo_lookup$areaname == input$geoname_simd]) &
  ...
  output$simd_text <- renderUI({
    ...
    if (is.na(simd_text_data$rii_int)) {
      ...
      tags$ul( #if no data available for PAR only first two points
        ...
        tags$li(class= "li-custom",  
                p(paste0("The most deprived areas have ", 
                         abs(round(unique(simd_text_data$rii_int), 0)),
                         "% ", 
                         more_less, 
                         tolower(unique(simd_text_data$label_ineq)), 
                         than_as, 
                         " the overall average." )))
                )
      ...
      tags$ul( 
        ...
        tags$li(class= "li-custom",  
                p(paste0("The most deprived areas have ", 
                         abs(round(unique(simd_bar_data()$rii_int), 0)), "% ", 
                         more_less, 
                         tolower(unique(simd_bar_data()$label_ineq)), 
                         than_as, 
                         " the overall average." ))),
                tags$li(class= "li-custom",
                        p(paste0(input$indic_simd, " would be ",
                                 abs(round(unique(simd_bar_data()$par), 0)),"% ", 
                                 par_more_less, "if the levels of the 
    ...
    simd_trend_data() %>% 
      ...
      mutate(rii_int = case_when(quintile != "Total" ~ NA_real_,
             ...
             par = case_when(quintile != "Total" ~ NA_real_,
                             TRUE ~ par)) %>% 
      rename(geography_code = code, 
             indicator_measure = measure,
             lower_confidence_interval=lowci, 
             upper_confidence_interval=upci,
             period = def_period, 
             definition = type_definition, 
             relative_inequality_gap = rii_int,
             lower_confidence_interval_relative_ineq = lowci_rii_int, 
             upper_confidence_interval_relative_ineq = upci_rii_int,
             absolute_inequality_gap = sii, 
             lower_confidence_interval_absolute_ineq = lowci_sii,
  ...
  output$simd_bar_plot <- renderPlotly({
    ...
    if (is.data.frame(simd_bar_data()) && nrow(simd_bar_data()) == 0)
    {
      ...
      if (input$ci_simd == FALSE) {  
        tooltip_simd <- paste0("Quintile ", 
                               simd_bar_data()$quintile, "<br>",
                               simd_bar_data()$trend_axis, ": ", 
                               simd_bar_data()$measure, "<br>", 
                               simd_bar_data()$type_definition)
      } else { 
        tooltip_simd <- paste0("Quintile ", 
                               simd_bar_data()$quintile, "<br>",
                               simd_bar_data()$trend_axis, ": ",
                               simd_bar_data()$measure, "<br>","95% confidence interval: ", 
                               simd_bar_data()$lowci, "-", 
                               simd_bar_data()$upci, "<br>", 
                               simd_bar_data()$type_definition)
      }
      ...
      p <- plot_ly(data=simd_bar_data(), 
        ...
        config(displayModeBar = F, 
               displaylogo = F, 
               editable =F) # taking out toolbar
    }

    if (input$ci_simd == FALSE) {  
      #adding bar layer without confidence intervals
      p %>% add_bars(y = ~measure, 
                     color = ~ quintile, 
                     marker = list(color = pal_simd_bar))
    } else { 
      ...
      plot_nodata()
        ...
        if (input$ci_simd == FALSE) {  
          tooltip_simd <- paste0(simd_trend_data()$quintile, "<br>",
                                 ...
                                 simd_trend_data()$type_definition)
        } else { 
          tooltip_simd <- paste0(simd_trend_data()$quintile, "<br>",
                                 simd_trend_data()$trend_axis, ": ", 
                                 simd_trend_data()$measure, "<br>", "95% confidence interval: ",
                                 simd_trend_data()$lowci, "-", 
                                 simd_trend_data()$upci, "<br>", 
                                 simd_trend_data()$type_definition)
        }
        ...
        #Creating plot
        trend_simd_plot <- plot_ly(data=simd_trend_data(), 
                                   x=~trend_axis, 
                                   text=tooltip_simd, 
                                   hoverinfo="text") %>%
          add_lines(y = ~measure, 
...
###############################################.
  ...
  output$title_sii <- renderUI({
    div(p(tags$b("Inequality gap over time")),
        p(paste0("Absolute differences between most and least deprived areas, 
                 expressed as ", 
                 tolower(unique(simd_trend_data()$type_definition)), ".")))
  })
  ...
  output$simd_sii_plot <- renderPlotly({
    ...
    # #Text for tooltips
      if (input$ci_simd == FALSE) {  
        tooltip_sii <- paste0(simd_index$trend_axis, ": ", 
                              simd_index$sii, "<br>",
                              simd_index$type_definition, "<br>", 
                               "Also known as Slope Index of Inequality")
      } else { 
        tooltip_sii <- paste0(simd_index$trend_axis, ": ", 
                              simd_index$sii, "<br>", "95% confidence interval: ",
                              simd_index$lowci_sii, "-", "<br>", 
                              simd_index$type_definition, "<br>", 
                              "Also known as Slope Index of Inequality")
      }
    ...
    #Create plot SII
    sii_plot <- plot_ly(data=simd_index, 
                        x=~trend_axis,
                        text=tooltip_sii, 
                        hoverinfo="text") %>%
      add_lines(y = ~sii, 
                name = "Absolute inequality (SII)", 
                type = 'scatter', 
                mode = 'lines',
  ...
  output$simd_rii_plot <- renderPlotly({
    ...
    if (is.na(simd_index$sii))
    {
      ...
      # #Text for tooltips
      if (input$ci_simd == FALSE) {  
        tooltip_rii <- c(paste0(simd_index$trend_axis, ": ", 
                                simd_index$rii_int, 
        ...
        tooltip_rii <- c(paste0(simd_index$trend_axis, ": ", 
                                ...
                                simd_index$lowci_rii_int, "-", 
                                simd_index$upci_rii_int, "<br>", 
                                "Also known as Relative Index of Inequality"))
      }
      ...
      #Create plot RII
      rii_plot <- plot_ly(data=simd_index, 
                          x=~trend_axis,
                          text=tooltip_rii, 
                          hoverinfo="text") %>%
        add_lines(y = ~rii_int, 
  ...
  output$simd_par_barplot_title <- renderUI({
    div(p(tags$b(paste0("Attributable to inequality, ", 
                        input$year_simd))),
        p("What part of ", 
          tolower(input$indic_simd), " can be attributed to socioeconomic inequalities."))
  })
  output$simd_par_trendplot_title <- renderUI({
    div(p(tags$b(paste0("Potential for improvement"))),
        p("How much ", 
        tolower(input$indic_simd), "could be reduced if the levels 
  ...
  output$simd_par_barplot <- renderPlotly({
    ...
    par_bar_plot <- plot_ly(data = simd_parbar_data, 
                            x = ~quintile, 
                            text=tooltip_parbar, 
                            hoverinfo="text") %>%
      add_bars(y = ~baseline, name= "", 
               marker = list(color = "#4da6ff"), 
               showlegend = FALSE) %>%   
      add_bars(y = ~diff_baseline, 
               name = "Attributable to deprivation", 
               marker = list(color = "#ffa64d"), 
               showlegend = FALSE) %>% 
      layout(bargap = 0.1, 
             barmode = 'stack', 
             showlegend = T, 
  ...
  #Title
  output$simd_par_trendplot_title <- renderUI({
    div(p(tags$b("Potential for improvement of ", 
                 tolower(input$indic_simd))),
  ...
  output$simd_par_trendplot <- renderPlotly({
    ...
    par_trend_plot <- plot_ly(data=simd_partrend_data, 
                              x=~trend_axis,
                              text=tooltip_partrend, 
                              hoverinfo="text") %>%
      add_lines(y = ~par, 
                type = 'scatter', 
                mode = 'lines', 
