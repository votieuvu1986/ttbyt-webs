...
output$geotype_ui_rank <- renderUI({
  ...
  selectInput("geotype_rank", 
              label = "Step 2. Select a geography level",
              choices = areas, 
              selected = georank_chosen$value_geo)
})
...
output$yearcomp_ui_rank <- renderUI({
  ...
  div(title="Use this option to change the baseline year (the black circle in chart)",
      selectInput("yearcomp_rank",
                  "Step 3b. Select comparison baseline year", 
                  choices = periods,
                  selectize=TRUE))
})
...
output$defs_text_rank <- renderUI({
  ...
  HTML(paste(sprintf("<b><u>%s</b></u> <br> %s ", 
                     defs_data_rank()$indicator_name, 
                     defs_data_rank()$indicator_definition), 
             collapse = "<br><br>"))
})
...
rank_bar_data <- reactive({
  ...
  if (input$geotype_rank %in% c("Scotland", 
                                "Health board", 
                                "Council area", 
...
plot_rank_charts <- function(){
  ...
  if (is.data.frame(rank_bar_data()) && nrow(rank_bar_data()) == 0)
  {
    ...
    tooltip_bar <-c(paste0(rank_bar_data()$areaname, ": ", 
                           rank_bar_data()$measure, "<br>",
                           input$geocomp_rank, ": ", 
                           rank_bar_data()$comp_value))
    ...
    if (input$comp_rank == 1) {
      #Comparator line
      rank_plot <- rank_plot %>% 
        add_trace(x = ~areaname, 
                  y = ~comp_value, 
                  name = ~unique(comp_name), 
                  type = 'scatter', 
                  mode = 'lines',
      ...
      if (input$ci_rank == FALSE) {  
        ...
        rank_plot <- rank_plot %>% add_bars(x = ~areaname, 
                                            y = ~ measure, 
                                            text=tooltip_bar, 
                                            hoverinfo="text",
      ...
      rank_plot <- rank_plot %>% 
        ...
        add_trace(y = ~areaname, 
                  ...
                  hoverinfo="text") %>% 
        # Adding layout
        layout(xaxis = list(title = ~type_definition, 
                            titlefont =list(size=14), 
                            side = "top", 
                            tickfont =list(size=14), 
                            fixedrange=TRUE),
...
output$download_rankplot <- downloadHandler(
  ...
  content = function(file){
    export(p = plot_rank_charts() %>% 
             layout(title = paste0(input$indic_rank, "<br>", 
                                   make_rank_subtitle()),
                    margin = list(t = 180)),
           file = file)
  })
...
output$defs_text_map <- renderUI({
  ...
  HTML(paste(sprintf("<b><u>%s</b></u> <br> %s ", 
                     defs_data_map()$indicator_name,
                     defs_data_map()$indicator_definition), 
             collapse = "<br><br>"))
})
...
output$map_ui <- renderUI({
  if(is.data.frame(poly_map()) && nrow(poly_map()) == 0) {
    h4("No map available for that geographic level.", 
       style = "color:black")
  } else {
...
output$download_map <- downloadHandler(filename =  'map_data.csv',
                                       content = function(file) { write.csv(map_csv(), 
                                                                            file, 
                                                                            row.names=FALSE)})
...
output$rank_legend <- renderUI({
  if (input$comp_rank == 1) {
    ...
    p(tags$b("Chart legend"), br(),
      ...
      "No differences can be calculated",
      br(),
      img(src='baseline_year_color.png', 
          height=12, 
          style="padding-right: 2px; vertical-align:middle"), 
